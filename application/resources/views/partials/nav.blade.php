      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          <li class="nav-item {{ (Route::is('home') ? 'active' : '') }}">
            <a class="nav-link" href="{{ route('home') }}">{{__('Transactions')}} @if(Route::is('home'))<span class="sr-only">(current)</span>@endif</a>
          </li>
          <li class="nav-item">
             <a href="{{url('/')}}/exchange/first/0/second/0"  class="nav-link  {{ (Route::is('exchange.form') ? 'active' : '') }}">{{__('Exchange')}} @if(Route::is('exchange.form'))<span class="sr-only">(current)</span>@endif</a>
          </li>
          <li class="nav-item">
            <a class="" href="#"></a>
           ...
           <li class="nav-item">
            <a href="{{route('withdrawal.index')}}"  class="nav-link  {{ (Route::is('withdrawal.index') ? 'active' : '') }}">{{__('My Withdrawals')}} @if(Route::is('withdrawal.index'))<span class="sr-only">(current)</span>@endif</a>
          </li>
        </ul>
