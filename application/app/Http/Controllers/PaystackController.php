...
class PaystackController extends Controller
{
	...
	public function buyvoucher(Request $request){
   	...
   	public function payVoucherPayStackSuccess(Request $request)
    {	
    	$curl = curl_init();
    ...
    public function sendRequestToPayStack(Request $request)
    {
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_POSTFIELDS => json_encode([
		    'amount'=>$amount,
		    'email'=>$email,
		    'callback_url'=>url('/').'/pay/voucher/paystack/success',
		  ]),
		  CURLOPT_HTTPHEADER => [
			...
			*/
			...
			curl_setopt_array($curl, array(
			  ...
			  CURLOPT_POSTFIELDS => json_encode([
			    'amount'=>$amount,
			    'email'=>$email,
		    	'callback_url'	=>	url('/').'/merchant/storefront/paystack/success?ref='.$PurchaseRequest->ref,
			  ]),
			  CURLOPT_HTTPHEADER => [
			    "authorization: Bearer $this->SECRET_KEY", //replace this with your own test key
			    "content-type: application/json",
