    ...
    public function index(Request $request)
    {
        ...
        if ($dataType->server_side) {
            ...
            foreach ($searchable as $key => $value) {
                ...
                $item = $dataRow->where('field', $value)->first();
                if($item == null) continue;//skipping metal_value because it is throwing error
        ...
        if($slug == 'withdrawals')
        {
            $obj = (object) array('name' => 'Confirm', 'style' => 'btn-primary');
            $extraActions[] = $obj;
        }

        $view = 'voyager::bread.browse';
        if (view()->exists("voyager::$slug.browse") && $slug!="users") {
